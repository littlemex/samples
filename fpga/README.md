...
AWS F2 FPGA インスタンス上で FPGA 開発環境を構築するための CloudFormation テンプレートと、汎用的なタスクランナーです。
...
## 汎用的なタスクランナー（Phase 0 セットアップ）

このプロジェクトでは、[Zenn の記事](https://zenn.dev/tosshi/articles/a18dce7d66424d) で紹介されている JSON ベースのタスク定義システムを採用した汎用的なタスクランナーを提供しています。

### アーキテクチャ

1. **`run-tasks.sh`**: 汎用タスクランナー
   - JSON タスク定義を読み込み、SSM Run Command 経由で実行
   - 冪等性を保証（失敗箇所から再開可能）
   - 進捗状態を `/tmp/task-state-{instance-id}.json` に永続化

2. **`tasks/phase0-setup.json`**: Phase 0 セットアップタスク定義
   - 9 つの独立したタスクで構成
   - aws-fpga のクローン、Vivado 設定、HDK セットアップ等

### 利点

従来の単一スクリプト方式と比較して：

- **高速な反復開発**: 設定変更時に CloudFormation スタックの再作成不要
- **透明性の高いログ**: コマンド実行結果が直接確認可能
- **粒度の高い制御**: 個別タスクの再実行やスキップが可能
- **失敗時の復旧**: 失敗したタスクから再開可能
...
export INSTANCE_ID=$(aws cloudformation describe-stacks \
  --stack-name fpga-phase0 \
  --region us-east-1 \
  --query 'Stacks[0].Outputs[?OutputKey==`InstanceId`].OutputValue' \
  --output text)

# Phase 0 セットアップを実行
./run-tasks.sh -i $INSTANCE_ID -f tasks/phase0-setup.json
```

#### 特定のタスクから再開

```bash
# タスク 05-setup-hdk から再開
./run-tasks.sh -i $INSTANCE_ID -f tasks/phase0-setup.json -s 05-setup-hdk
```
...
```bash
# 実際には実行せず、タスク内容のみ表示
./run-tasks.sh -i $INSTANCE_ID -f tasks/phase0-setup.json --dry-run
```
...
# Vivado バージョンを変更して実行
./run-tasks.sh -i $INSTANCE_ID -f tasks/phase0-setup.json \
  -v '{"VIVADO_VERSION": "2024.2","VIVADO_PATH": "/opt/Xilinx/2024.2/Vivado"}'
```

### Phase 0 タスク一覧

1. **01-clone-aws-fpga**: aws-fpga リポジトリのクローン
2. **02-configure-vivado**: Vivado パスの設定
3. **03-patch-vivado-version**: Vivado バージョンの互換性設定
4. **04-configure-git**: Git の所有権設定
5. **05-setup-hdk**: HDK 環境のセットアップ
6. **06-add-to-bashrc**: .bashrc への追加
...
- [Zenn 記事: SSM Run Command による汎用的なタスクランナー](https://zenn.dev/tosshi/articles/a18dce7d66424d)
- [GitHub: littlemex/samples/aws-neuron](https://github.com/littlemex/samples/tree/main/aws-neuron/torch-neuronx/multi-framework-dlami-ubuntu24-cdk)
...
F2 インスタンスのコストは以下の通りです（us-east-1、オンデマンド価格、2025 年 1 月時点）：

| インスタンスタイプ | 時間単価 | 月額（730 時間） |
|-----------------|---------|----------------|
