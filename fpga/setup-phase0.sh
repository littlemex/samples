...
if [ "$LOCAL_MODE" = true ]; then
  ...
  REMOTE_SCRIPT="$SCRIPT_DIR/setup-phase0-remote.sh"

  if [ ! -f "$REMOTE_SCRIPT" ]; then
    log_error "Remote script not found: $REMOTE_SCRIPT"
    exit 1
  fi

  # Encode script to base64
  SCRIPT_B64=$(base64 -w 0 < "$REMOTE_SCRIPT")

  # Execute via SSM Run Command
  log_info "Sending script to instance via SSM Run Command..."
  COMMAND_ID=$(aws ssm send-command \
    --instance-ids "$INSTANCE_ID" \
    --region "$REGION" \
    --document-name "AWS-RunShellScript" \
    --comment "Phase 0: FPGA Simulation Setup" \
    --parameters "{\"commands\":[\"echo '$SCRIPT_B64' | base64 -d > /tmp/setup.sh\",\"chmod +x /tmp/setup.sh\",\"bash /tmp/setup.sh\"]}" \
    --output json \
