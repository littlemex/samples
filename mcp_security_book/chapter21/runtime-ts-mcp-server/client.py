...
class MCPClient:
    ...
    async def send_request(self, payload: dict) -> dict:
        ...
        self.logger.debug(f"Sending request: {payload.get('method', 'unknown')} (ID: {payload.get('id', 'N/A')})")
...
def parse_args():
    ...
    group.add_argument('--local', action='store_true', help='Connect to localhost:8000/mcp')
    group.add_argument('--remote', action='store_true', help='Connect to AWS Bedrock AgentCore Runtime')
