...
インスタンスタイプ（Neuron対応インスタンスのみ）:
    • inf2.* (例: inf2.xlarge, inf2.8xlarge, inf2.24xlarge, inf2.48xlarge)
    • trn1.* (例: trn1.2xlarge, trn1.32xlarge)
    • trn1n.* (例: trn1n.32xlarge)
    • trn2.* (例: trn2.3xlarge, trn2.8xlarge, trn2.16xlarge) ※trn2u.*は除く

使用例:
    # テンプレートをS3にアップロード
    $0 upload

    # 基本的な作成（デフォルト: us-east-1, trn1.2xlarge）
    $0 create

    # リージョンとインスタンスタイプを指定して作成
    $0 create -r us-west-2 -t inf2.8xlarge

    # カスタム設定で作成
    $0 create -n my-vscode -r us-east-1 -t trn1.32xlarge -u developer
...
validate_instance_type() {
    ...
    # 許可されたインスタンスタイプのパターン
    # Inf2: inf2.*
    # Trn1: trn1.* (trn1.2xlarge, trn1.32xlarge)
    # Trn1n: trn1n.* (trn1n.32xlarge)
    # Trn2: trn2.* (trn2u.*は除外)
    
    if [[ $instance_type =~ ^inf2\. ]]; then
        log_info "✓ Inf2インスタンスタイプが選択されました: $instance_type"
        return 0
    elif [[ $instance_type =~ ^trn1\. ]]; then
        log_info "✓ Trn1インスタンスタイプが選択されました: $instance_type"
        return 0
    elif [[ $instance_type =~ ^trn1n\. ]]; then
        log_info "✓ Trn1nインスタンスタイプが選択されました: $instance_type"
        return 0
    elif [[ $instance_type =~ ^trn2\. ]] && [[ ! $instance_type =~ ^trn2u\. ]]; then
        log_info "✓ Trn2インスタンスタイプが選択されました: $instance_type"
        return 0
    else
        log_error "❌ サポートされていないインスタンスタイプ: $instance_type"
        log_error "許可されているインスタンスタイプ:"
        log_error "  • inf2.* (例: inf2.xlarge, inf2.8xlarge, inf2.24xlarge, inf2.48xlarge)"
        log_error "  • trn1.* (例: trn1.2xlarge, trn1.32xlarge)"
        log_error "  • trn1n.* (例: trn1n.32xlarge)"
        log_error "  • trn2.* (例: trn2.3xlarge, trn2.8xlarge, trn2.16xlarge) ※trn2u.*は除く"
        exit 1
    fi
}
