...
インスタンスタイプ（GPU対応インスタンスのみ）:
    • g5.* (例: g5.xlarge, g5.2xlarge, g5.4xlarge, g5.8xlarge, g5.12xlarge, g5.16xlarge, g5.24xlarge, g5.48xlarge)
    • g6.* (例: g6.xlarge, g6.2xlarge, g6.4xlarge, g6.8xlarge, g6.12xlarge, g6.16xlarge, g6.24xlarge, g6.48xlarge)
...
使用例:
    ...
    $0 create -r us-west-2 -t g5.2xlarge

    # カスタム設定で作成
    $0 create -n my-vscode -r us-east-1 -t g6.8xlarge -u developer
...
機能:
    ...
    🎮 GPU DLAMI (PyTorch 2.9 + NVIDIA Driver) が事前インストール済み
    📦 ネストされたスタックでサイズ制限を回避
EOF
}
...
validate_instance_type() {
    ...
    # 許可されたインスタンスタイプのパターン
    # G5: g5.* (NVIDIA A10G GPU)
    # G6: g6.* (NVIDIA L4 GPU)
    
    if [[ $instance_type =~ ^g5\. ]]; then
        log_info "✓ G5インスタンスタイプが選択されました: $instance_type"
        return 0
    elif [[ $instance_type =~ ^g6\. ]]; then
        log_info "✓ G6インスタンスタイプが選択されました: $instance_type"
        return 0
    else
        log_error "❌ サポートされていないインスタンスタイプ: $instance_type"
        log_error "許可されているインスタンスタイプ:"
        log_error "  • g5.* (例: g5.xlarge, g5.2xlarge, g5.4xlarge, g5.8xlarge, g5.12xlarge)"
        log_error "  • g6.* (例: g6.xlarge, g6.2xlarge, g6.4xlarge, g6.8xlarge, g6.12xlarge)"
        exit 1
    fi
}
